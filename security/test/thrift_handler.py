...
from ezbake.base.thriftapi.ttypes import (EzSecurityToken, TokenRequest, TokenType, EzSecurityPrincipal, ValidityCaveats,
                                       CommunityMembership, Authorizations)
...
class EzSecurityHandler(object):
    def __init__(self, rsa, token_ttl_millis=60000):
        self.app_name = "common_services"
        self.service_name = SECURITY_SERVICE_NAME
        self.rsa = ossl.load_privatekey(ossl.FILETYPE_PEM, rsa)

        self.token_ttl_millis = token_ttl_millis
        ...
        try:
            ...
            if token:
                validity_caveats = ValidityCaveats(
                    ...
                    notAfter=util.current_time_millis() + self.token_ttl_millis
                )
